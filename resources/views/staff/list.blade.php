...
@if($permission->staff_role === 'admin' || $permission->staff_role === 'user')
    ...
    <div class="m-portlet m-portlet--full-height m-portlet--tabs" id="filter_bar">
        <div class="row" style="margin-top: 20px;">
            ...
            <div class="col justify-content-end">
                    <a  class="btn btn--custom ml-1" href="{{route('staff_event.calendar')}}"
                ...
                @if($permission->staff_role === 'admin')
                    <a  class="btn btn--custom ml-1" href="{{url('staff_event/create')}}"
...
@push('scripts')
    ...
    <script>
            ...
            columns: [
                ...
                { data: 'staff_pic', render:function (staff_pic, _, row) {
                        if (staff_pic) {
                            ...
                            return `<div class="square text-center" style="background-color: #e0e0e0">
                                    <img src="{{ url('/uploads')  }}/images/no_image.png"   target="_blank" class="img-fluid" style="height: 100%;"></div>`;
                        }
                    },width: '10%'},
        ...
        var t_event = $('#staff-event-table').DataTable({
            ...
            bDeferRender: true,
            ajax: {
                url: '{!! route('staff_event.get_event') !!}',
                type: 'GET',
                data: function (d) {
                    d._token = "{{ csrf_token() }}",
                        d.search = $('#data-search').val()
...
@endpush
@else
    <script>window.location = "{{route('staff_event.calendar')}}";</script>
@endif
