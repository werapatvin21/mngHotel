    ...
    <div class="m-portlet m-portlet--full-height m-portlet--tabs" id="filter_bar">
        <div class="row" style="margin-top: 20px;">
            ...
            <div class="col-2">
                ...
                <a class="btn btn--custom" href="{{route('reservation.index')}}"
    ...
    <div class="m-portlet m-portlet--full-height m-portlet--tabs">
                        ...
                        <div class="col-sm-12">
                            <div class="container">
                                ...
                                <form class="m-form m-form--fit " enctype="multipart/form-data"
                                      action="{{ $data->id ? route('reservation.update', ['id' => $data->id]) : route('reservation.store') }}"
                                      method="POST">
                                    @csrf
                                    @if($data && $data->id)
                                        @method('PUT')
                                            ...
                                            <div class="form-group row">
                                                ...
                                                <div class="col-sm-8">
                                                    ...
                                                    <input id="check_in" name="check_in"
                                                           value="{{ $check_in >= date('Y-m-d') ? $check_in : ''}}"
                                            ...
                                            <div class="form-group row">
                                                ...
                                                <div class="col-sm-8">
                                                    <input type="number" onchange="clearStorage()" min="1" name="total_night" class="form-control"
                                                           id="total_night" value="{{$data->total_night?: 1}}" required>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    {{--<select class="form-control" name="room_id" id="room_id" required>--}}
                                        ...
                                        {{--@if(isset($rooms)&& count($rooms) != 0)--}}
                                            {{--@php $room_id = old('room_id',$data->room_id); @endphp--}}
                                            {{--@foreach($rooms as $key => $room)--}}
                                                {{--<option {{$room->id === $room_id ?'selected':''}} value="{{$room->id}}"--}}
                                                        {{--@if($room->status === 'unavailable' || $room->status === 'maintenance')--}}
                                                            {{--@if(!$data->id)--}}
                                                                {{--disabled--}}
                                                            {{--@elseif($room->id != $data->room_id)--}}
                                                                {{--disabled--}}
                                                            {{--@endif--}}
                                                        {{--@endif--}}
                                                {{-->--}}
                                                    {{--{{$room->name}}--}}
                                                    {{--@if($room->status === 'unavailable' && $room->id != $data->room_id)--}}
                                                        {{--<span style="color: red">(ไม่ว่าง)</span>--}}
                                                    {{--@elseif($room->status === 'maintenance' && $room->id != $data->room_id)--}}
                                                        {{--<span style="color: red">(ซ่อมบำรุง)</span>--}}
                                                    {{--@else--}}
                                                        {{--@if($data->room_id && $room->id == $data->room_id)--}}
                                                            {{--<span style="color: green">ใช้งานอยู่</span>--}}
                                                        {{--@else--}}
                                            ...
                                            <div class="col-sm-10">
                                                <select class="form-control" onchange="clearStorage()" name="room_id" id="room_id" required>
                                                    ...
                                                    @if(isset($rooms)&& count($rooms) != 0)
                                                        @php $room_id = old('room_id',$data->room_id); @endphp
                                                        @foreach($rooms as $key => $room)
                                                            <option id="{{$room->id.'room'}}" {{$room->id === $room_id ?'selected':''}} value="{{$room->id}}">
                                                                {{$room->name}}
                                                            </option>
                                                        @endforeach
                                                    @else
                                                    ...
                                                    <div class="input-group">
                                                        <select style="width: 60%" class="form-control" name="id_guest"
                                                            ...
                                                            @if(isset($guests)&& count($guests) != 0)
                                                                @php $id_guest = old('room_id',$data->id_guest); @endphp
                                                                @foreach($guests as $key => $guest)
                                                                    <option {{$guest->id === $id_guest ?'selected':''}} value="{{$guest->id}}">{{$guest->first_name}}   {{$guest->last_name}} </option>
                                                                @endforeach
                                                            @else
                                 ...
                                 {{--   @if(isset($data))
                                    <div class="row">
                                        <div class="col-6">
                                            @foreach($reservServ as $key => $rs)
                                                @if ($key > 0)
                                                    <select class="form-control" name="service{{$key}}" id="services" required>
                                                        <option value="">-- Service Name --</option>
                                                        <!-- ต้องดึงข้อมูลที่ผู้ใช้สร้าง ขึ้นมาแสดง -->
                                                        @if(isset($services)&& count($services) != 0)
                                                            @php $service_id = old('services',$rs->id_services); @endphp
                                                            @foreach($services as $key => $service)
                                                                <option id="{{$service->id}}" {{$service->id === $service_id ?'selected':''}} value="{{$service->id}}">
                                                                    {{$service->name}}
                                                                </option>
                                                            @endforeach
                                                        @else
                                    ...
                                    <div class="row">
                                        <div class="col">
                                            <div class="form-group row">
                                                ...
                                                <div class="col-sm-8"><input type="text" value="{{$data->price?:0}}"
                                        ...
                                        <div class="col">
                                            <div class="form-group row">
                                                ...
                                                {{--<label for="discount" class="col-sm-4 col-form-label">--}}
                                                    {{--Discount</label>--}}
                                                {{--<div class="col-sm-3">--}}
                                                        {{--<input type="text"  id="promotion_code" readonly name="promotion_code" value="{{$data->promotion_code}}"  class="form-control">--}}
                                                    {{--</div>--}}
                                        ...
                                        <div class="col">
                                            <div class="form-group row">
                                                ...
                                                <div class="col-sm-8">
                                                    ...
                                                    @php $id_gent = old('id_gent',$data->id_gent); @endphp
                                                    @if(isset($agents)&& count($agents) != 0)
                                                        @foreach($agents as $key => $agent)
                                                            <option {{$agent->id === $id_gent ?'selected':''}} value="{{$agent->id}}">{{$agent->name}} </option>
                                                        @endforeach
                                                    @else
                                        ...
                                        <div class="col">
                                            <div class="form-group row">
                                                ...
                                                <select for="req" class="form-control" id="special_request"
                                                    ...
                                                    <option @if($data->special_request) selected @endif   value="yes">
                                                        Special Request
                                                    </option>
                                                </select>
                                                </div>
                                            ...
                                            <div class="col">
                                                <div class="form-group row">
                                                    ...
                                                    <div class="col-sm-7" style="">
                                                        <input type="text"  style="text-align:right;" width="50px" id="total_price" readonly name="total_price" value="{{$data->total_price?: 0}}"  class="form-control">
                                                    </div>
                                                    <div class="col-sm-1">
                                                        <b>฿</b>
                                                    </div>
                                        ...
                                        <div class="col">
                                            <div class="form-group row">
                                                <textarea name=special_request_note" id="special_request_note"
                                                          ...
                                                          placeholder="Typing Special Request Here"> {{$data->special_request}}</textarea>
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-6">
                                            {{-- <select class="form-control" name="service0" id="services" required>
                                                 <option value="">-- Service Name --</option>
                                                 <!-- ต้องดึงข้อมูลที่ผู้ใช้สร้าง ขึ้นมาแสดง -->
                                                 @if(isset($services)&& count($services) != 0)
                                                     @php if (isset($reservServ[0]->id_services)) {$service_id = old('service0',$reservServ[0]->id_services);} @endphp
                                                     @foreach($services as $key => $service)
                                                         <option id="{{$service->id}}" {{(isset($service_id) && $service->id === $service_id) ?'selected':''}} value="{{$service->id}}">
                                                             {{$service->name}}
                                                         </option>
                                                     @endforeach
                                                 @else
                                    ...
                                    @if(isset($reservation_services) &&  count($reservation_services) > 0)
                                        <div class="row">
                                            <div class="col-12">
                                                <table class="table">
                                                    <thead>
                                                    ...
                                                    <tbody>
                                                    <?php $total_reservServ = 0?>
                                                    @foreach($reservation_services as $index => $Serv)
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <a href="{{url('reservation').'/'.$data->id.'?reservServ='.$Serv->id}}">
                                                                    <button type="button" onclick="deleteService({{ $Serv->id_services }})" class="btn btn-danger btn-circle">
                                                    ...
                                                    @endforeach
                                                    {{--<tr>--}}
                                                        {{--<td colspan="5" align="right"> Total/Service </td>--}}
                                                        {{--<td colspan="">--}}
                                                            {{--<b>{{(Float) $total_reservServ}} ฿</b>--}}
                                                        {{--</td>--}}
                                                    {{--</tr>--}}
        ...
        @push('scripts')
            ...
            <script>
                ...
                var index = 0;
                if (data_edit) {
                    index = {!! count($reservServ->toArray()) !!}
                    $('#numServ').val(index);
                }
                                    ...
                                    `<div class="row service${index}">
                                        <div class="col-6">
                                            <select class="form-control" name="service${index}" onchange='addServicePrice("service${index}")' id="service${index}" required>
                                                <option value="">-- Service Name --</option>
                                                    @if(isset($services)&& count($services) != 0)
                                                        @php $service_id = old('services',$data->id_services); @endphp
                                                        @foreach($services as $key => $service)
                                                        <option id="{{$service->id}}" {{$service->id === $service_id ?'selected':''}} value="{{$service->id}}">
                ...
                function deleteService(service_id) {
                    $.ajax({
                        type: 'post',
                        data: {'serviceId': service_id, "_token": "{{ csrf_token() }}"},
                        url: '{!! route('get_price_service') !!}',
                        dataType: 'json',
                ...
                function deleteSelect(index, serviceId) {
                    ...
                    $.ajax({
                        type: 'post',
                        data: {'serviceId': id, "_token": "{{ csrf_token() }}"},
                        url: '{!! route('get_price_service') !!}',
                        dataType: 'json',
                        success: function (res) {
                            var total = parseFloat($("#total_price").val())-res.data.price;
            ...
            <script>
                var reservation = JSON.parse('{!! json_encode($reservations) !!}');
                var rooms = JSON.parse('{!! json_encode($rooms) !!}');
                var data = JSON.parse('{!! json_encode($data) ?: 0 !!}');
                var data_id = {!! $data->id?:null ?: 0 !!}
                $(function () {
                    if (data && data_id) {
                ...
                $('#total_night').change(function () {
                    var total_service = '{!! $total_reservServ !!}';
...
// console.log( $("#room_id").val())
                        ...
                        if(length >= 2){
                            $.ajax({
                                type: 'post',
                                data: {'code': promotion_code, "_token": "{{ csrf_token() }}"},
